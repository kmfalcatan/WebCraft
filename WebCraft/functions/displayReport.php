...
if ($report_ID) {
    ...
    if ($result_info && mysqli_num_rows($result_info) > 0) {
        ...
        $query_data = "SELECT unit_ID, equipment_ID, user_ID, report_issue, problem_desc FROM unit_report WHERE timestamp = '$timestamp'";
...
$user_full_name = '';
if ($user_ID) {
    $query_user_info = "SELECT fullname FROM users WHERE id = '$user_ID'";
    ...
    if ($result_user_info && mysqli_num_rows($result_user_info) > 0) {
        ...
        $user_full_name = $row_user_info['fullname'];
    }
}
...
foreach ($unit_reports as $report) {
    ...
    $query_equipment_info = "SELECT deployment, property_number, account_code, description, image, article FROM equipment WHERE equipment_ID = '$equipment_ID'";
    ...
    if ($result_equipment_info && mysqli_num_rows($result_equipment_info) > 0) {
        ...
        $description = $row_equipment_info['description'];
